...
from intrinseca.core.engine import Engine, EngineConfig, EngineStats, DayResult
...
@pytest.fixture
def basic_config(tmp_silver_path):
...
class TestEngineCreation:
    ...
    def test_engine_basic_creation(self, basic_config):
        """Test creación básica."""
        engine = Engine(basic_config)
        
        assert engine.config is basic_config
        assert engine.stats is not None
    
    def test_engine_warmup(self, basic_config):
...
class TestEngineProcessDay:
    ...
    def test_process_day_returns_day_result(self, basic_config, sample_bronze_df):
    ...
    def test_process_day_counts_ticks(self, basic_config, sample_bronze_df):
    ...
    def test_process_day_measures_time(self, basic_config, sample_bronze_df):
    ...
    def test_process_day_creates_parquet(self, basic_config, sample_bronze_df):
        ...
        expected_path = (
            basic_config.silver_base_path / "TEST" / f"theta={basic_config.theta_str}" /
            "year=2025" / "month=06" / "day=15" / "data.parquet"
        )
        
        assert expected_path.exists()
    
    def test_process_day_parquet_readable(self, basic_config, sample_bronze_df):
        ...
        parquet_path = (
            basic_config.silver_base_path / "TEST" / f"theta={basic_config.theta_str}" /
            "year=2025" / "month=01" / "day=01" / "data.parquet"
        )
    ...
    def test_process_day_updates_stats(self, basic_config, sample_bronze_df):
...
class TestEngineProcessDateRange:
    ...
    def test_process_date_range_returns_dict(self, basic_config, multi_day_bronze_df):
        """Test que retorna diccionario de resultados."""
        engine = Engine(basic_config)
    ...
    def test_process_date_range_processes_all_days(self, basic_config, multi_day_bronze_df):
    ...
    def test_process_date_range_creates_files(self, basic_config, multi_day_bronze_df):
        ...
        for day in range(1, 6):
            path = (
                basic_config.silver_base_path / "TEST" / f"theta={basic_config.theta_str}" /
                "year=2025" / "month=01" / f"day={day:02d}" / "data.parquet"
            )
...
class TestEngineDiagnostics:
    ...
    def test_reset_stats(self, basic_config, sample_bronze_df):
    ...
    def test_diagnose_method(self, basic_config, sample_bronze_df):
