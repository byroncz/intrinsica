...
class TestFindPreviousState:
    ...
    def test_finds_previous_day(self, state_with_orphans, tmp_path):
        ...
        path = build_state_path(tmp_path, "TEST", 0.01, date(2025, 6, 15))
    ...
    def test_finds_with_gap(self, state_with_orphans, tmp_path):
        ...
        path = build_state_path(tmp_path, "TEST", 0.01, date(2025, 6, 10))
        path.parent.mkdir(parents=True, exist_ok=True)
        save_state(state_with_orphans, path)
...
class TestStateDiagnostics:
    ...
    def test_list_available_states(self, state_with_orphans, tmp_path):
        ...
        for day in [10, 15, 20]:
            path = build_state_path(tmp_path, "TEST", 0.01, date(2025, 6, day))
            path.parent.mkdir(parents=True, exist_ok=True)
            save_state(state_with_orphans, path)
        
        states = list_available_states(tmp_path, "TEST", 0.01)
        
        assert len(states) == 3
    
    def test_get_state_stats(self, state_with_orphans, tmp_path):
        """Test obtener estadísticas de estado."""
        path = tmp_path / "test_state.arrow"
        save_state(state_with_orphans, path)
        
        stats = get_state_stats(path)
        
        assert stats is not None
        assert "n_orphans" in stats
        assert stats["n_orphans"] == 4
