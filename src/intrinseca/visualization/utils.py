...
def _prepare_price_data(df: pl.DataFrame) -> pd.DataFrame:
    df_proc = df.with_columns(
        [
            ...
            pl.when(pl.col("dc_interval_type") == "Upward Overshoot")
            .then(pl.lit("Upward"))
            .when(pl.col("dc_interval_type") == "Downward Overshoot")
            .then(pl.lit("Downward"))
            .when(pl.col("dc_interval_type") == "Upturn Event")
            .then(pl.lit("Upturn"))
            .when(pl.col("dc_interval_type") == "Downturn Event")
            .then(pl.lit("Downturn"))
